...
# Use environment variables from your main platform
API_KEY   = os.getenv("MARKET_FEED_API_KEY", "Q07jqkk1")
CLIENT_ID = os.getenv("ANGEL_ONE_CLIENT_ID", "AACE648379")
PASSWORD  = os.getenv("ANGEL_ONE_PASSWORD", "2607")
TOTP_KEY  = os.getenv("ANGEL_ONE_TOTP_SECRET", "YPNWTO32HOA7IZ5KFNSMBZUQCE")
...
def fetch_ltp(token_str):
    try:
        ...
        # Fix: SmartAPI ltpData signature is (exchange, symboltoken, symboltype)
        try:
            resp = obj.ltpData("MCX", str(token_str), "")
            if resp and resp.get("status"):
                ltp = float((resp.get("data") or {}).get("ltp", 0))
                if ltp > 0:
                    return ltp, None
        except Exception as e:
            print(f"ltpData error: {e}")
        # Fallback: market data REST quote API
        ...
        # Only clear session on auth/token errors, not on market-closed etc.
        if any(x in msg.lower() for x in ["invalid token", "unauthorised", "unauthorized", "jwt", "expired"]):
            _session.clear()
        return None, msg
    except requests.exceptions.Timeout:
        # Don't clear session on transient network timeouts
        return None, "LTP request timed out"
    except Exception as e:
        # Only clear session if it looks like an auth failure
        err = str(e).lower()
        if any(x in err for x in ["invalid token", "unauthorised", "unauthorized", "jwt"]):
            _session.clear()
        return None, str(e)
...
def calculate_volatility(token_str):
    try:
        ...
        atr_abs    = round((atr_pct / 100) * close, 2) if close > 0 else 0.0
        print(f"Volatility: ATR%={atr_pct:.3f} source={atr_source} tick_vol%={tick_vol:.3f} level={level}")
        return {
            "level":      level,
            "atr_pct":    round(atr_pct, 3),
            "atr_abs":    atr_abs,        # absolute Rs ATR — used by frontend vol detail panel
            "atr_source": atr_source,
            "method":     atr_source,     # alias — frontend reads vol.method
        ...
        return {"level": "NORMAL", "atr_pct": 0, "atr_abs": 0, "atr_source": "none", "method": "none",
                "tick_vol": 0, "high": 0, "low": 0, "close": 0, "open": 0, "ticks_used": 0}
...
def pick_best_contract(symbol):
    try:
        ...
        for fut in candidates:
            ...
            ltp_q = float(q.get("ltp", 0) or 0)
            print(f"  {fut.get('tradingsymbol')}: vol={vol} ltp={ltp_q} days={fut['_days_to_expiry']}")
            if vol > best_vol or (best is None):
                best_vol = vol
                best = {
        ...
        # If no quote data at all, fall back to nearest-expiry contract with zero fields
        if best is None:
            fut  = valid[0]
...
if __name__ == "__main__":
    print("=" * 57)
    print("  MCX Silver Smart Allocator  ->  http://localhost:5001")
    print("  Interactive Docs            ->  http://localhost:5001/docs")
    print("=" * 57)
